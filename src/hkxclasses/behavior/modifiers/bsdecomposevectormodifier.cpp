...
void BSDecomposeVectorModifier::setW(const qreal &value){
    std::lock_guard <std::mutex> guard(mutex);
    if (value != w) {
        w = value, setIsFileChanged(true);
    } else {
        LogFile::writeToLog(getClassname()+": 'w' was not set!");
    }
}
...
void BSDecomposeVectorModifier::setZ(const qreal &value){
    std::lock_guard <std::mutex> guard(mutex);
    if (value != z) {
        z = value, setIsFileChanged(true);
    } else {
        LogFile::writeToLog(getClassname()+": 'z' was not set!");
    }
}
...
void BSDecomposeVectorModifier::setY(const qreal &value){
    std::lock_guard <std::mutex> guard(mutex);
    if (value != y) {
        y = value, setIsFileChanged(true);
    } else {
        LogFile::writeToLog(getClassname()+": 'y' was not set!");
    }
}
...
void BSDecomposeVectorModifier::setX(const qreal &value){
    std::lock_guard <std::mutex> guard(mutex);
    if (value != x) {
        x = value, setIsFileChanged(true);
    } else {
        LogFile::writeToLog(getClassname()+": 'x' was not set!");
    }
}
...
void BSDecomposeVectorModifier::setVector(const hkQuadVariable &value){
    std::lock_guard <std::mutex> guard(mutex);
    if (value != vector) {
        vector = value, setIsFileChanged(true);
    } else {
        LogFile::writeToLog(getClassname()+": 'vector' was not set!");
    }
}
