...
void hkbVariableBindingSet::removeBinding(const QString & path){
    ...
    for (auto i = 0; i < bindings.size(); i++){
        if (bindings.at(i).memberPath == path) {
            bindings.removeAt(i), setIsFileChanged(true);
        } else {
            LogFile::writeToLog(getClassname()+"removeBinding(): Binding was not removed!!");
        }
    }
}
...
QString hkbVariableBindingSet::getPathOfBindingAt(int index){
    ...
    if (index < bindings.size() && index >= 0) {
        ...
        LogFile::writeToLog(getClassname()+"getPathOfBindingAt(): Invalid binding index!!");
    }
    return path;
}
